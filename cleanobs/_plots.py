...
hv.opts.defaults(
    hv.opts.Curve(responsive=True, show_grid=True, tools=["hover", "crosshair", "undo"]),
    hv.opts.Path(responsive=True, show_grid=True),
    hv.opts.RGB(responsive=True, show_grid=True),
    hv.opts.Image(responsive=True, show_grid=True),
    hv.opts.Scatter(responsive=True, show_grid=True),
...
def show(*objs, threaded=True, **kwargs) -> pn.StoppableThread | pn.Server:
...
def dshow(*objs, threaded=True, **kwargs):
    ...
    kwargs["threaded"] = threaded
    return pn.serve(
        pn.Column(*[datashade(obj) for obj in objs]),
        **kwargs
    )
...
def rshow(*objs, threaded=True, **kwargs):
    ...
    kwargs["threaded"] = threaded
    return pn.serve(
        pn.Column(*[rasterize(obj) for obj in objs]),
        **kwargs
    )


def get_rolling_era5_msl(era5: pd.DataFrame, days: int = 3):
    era5_rolling = era5[["msl"]].rolling(pd.Timedelta(days=days), center=True).min().assign(value=0).reset_index()
    ...
    path = path.opts(
        ...
        cmap="reds",
        color=hv.dim("msl"),  # duh... https://github.com/holoviz/holoviews/issues/4862#issuecomment-1800276275
        clim=(100_000, 96_000),
        yaxis=False,
        xaxis=False,
...
def get_rolling_era5_wind(era5: pd.DataFrame, days: int = 3):
    era5_rolling = era5[["wind_mag"]].rolling(pd.Timedelta(days=days), center=True).max().assign(value=0).reset_index()
    ...
    path = path.opts(
        ...
        colorbar_opts={"width": 500, "location": "top_center", "height": 10},
        cmap="blues",
        color=hv.dim("wind_mag"),  # duh... https://github.com/holoviz/holoviews/issues/4862#issuecomment-1800276275
        clim=(10, 20),
        yaxis=False,
        xaxis=False,
        gridstyle={"ygrid_line_width": 0},
...
def _on_add_date_range(sr, trans, selection):
    if selection.index:
        trans.add_date_range(
            start=sr.index[selection.index[0]],
            end=sr.index[selection.index[-1]],
        )
...
def clean(
    ...
    compare_button.on_click(lambda _: compare(unique_id=unique_id, start=start, months=months, offset=offset))
