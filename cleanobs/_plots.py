...
def clean(
    ...
    if include_surge:
        ...
        dft = calc_surge(dft.loc[inner_start:inner_end], const, "utide")
    ...
    return show(
        pn.Row(
            ...
            pn.HSpacer(width_policy="max"),
            pn.pane.Markdown(
                f"## {unique_id}\n#### {inner_start.strftime('%Y-%m')} - {inner_end.strftime('%Y-%m')}",
            ),
            pn.HSpacer(width_policy="max"),
            compare_button,
            serialize_button,
        ...
        surge_curve if surge_curve else None,
    )
